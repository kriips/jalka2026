defmodule Jalka2026.Football do
  ...
  """
  def update_competition(%Competition{} = competition, attrs), do: Competitions.update(competition, attrs)

  # Mapping from FIFA team codes (used in teams table) to ISO 3166-1 alpha-3 codes (used in historical data)
  @fifa_to_iso %{
    "GER" => "DEU",
    "NED" => "NLD",
    "POR" => "PRT",
  ...
  """
  def fifa_to_iso_code(fifa_code) do
    Map.get(@fifa_to_iso, fifa_code, fifa_code)
  end
  ...
  def get_team_by_name(team_name) do
    Jalka2026.Football.Cache.get_team_by_name(team_name)
  end
  ...
  """
  def get_team(id) do
    Jalka2026.Football.Cache.get_team(id)
  end
  ...
  def change_score(
    ...
    result = TelemetryEvents.span_group_prediction(metadata, fn ->
  ...
  def update_playoff_result(phase, team_id) do
    case get_playoff_result_by_phase_team(phase, team_id) do
      ...
      nil ->
        %PlayoffResult{}
        |> PlayoffResult.create_changeset(%{phase: phase, team_id: team_id, competition_id: competition_id()})
        |> Repo.insert!()
    end
  end
  ...
  def get_team_world_cup_eliminations(team_code) do
    ...
    top4_years = Enum.map(top4_tournament_ids, fn id ->
  ...
  def add_favorite_team(user_id, team_id, is_primary \\ false) do
    ...
    %UserFavoriteTeam{}
    |> UserFavoriteTeam.changeset(%{user_id: user_id, team_id: team_id, is_primary: is_primary})
    |> Repo.insert()
  end
  ...
  """
  def is_favorite_team?(user_id, team_id) do
    query =
      from(uft in UserFavoriteTeam,
        where: uft.user_id == ^user_id and uft.team_id == ^team_id
      )
      ...
      stats =
        ...
        |> Enum.reduce(
          ...
          fn pred, acc ->
            match = pred.match
            involves_favorite =
              pred.match.home_team_id in favorite_team_ids or
              pred.match.away_team_id in favorite_team_ids
            ...
            if involves_favorite do
              %{
                ...
                | favorite_predictions: acc.favorite_predictions + 1,
                  favorite_correct_results: acc.favorite_correct_results + if(correct_result, do: 1, else: 0),
  ...
  """
  def is_rival?(user_id, rival_id) do
    query =
      from(ur in UserRivalry,
  ...
  def get_differing_predictions(user_id, rival_id) do
    ...
    |> Enum.map(fn match_id ->
      ...
      if user_pred && rival_pred do
        match = user_pred.match

        different_result = user_pred.result != rival_pred.result

        if different_result and not match.finished do
          %{
            match: match,
            user_prediction: %{home_score: user_pred.home_score, away_score: user_pred.away_score, result: user_pred.result},
            rival_prediction: %{home_score: rival_pred.home_score, away_score: rival_pred.away_score, result: rival_pred.result}
          }
        else
          nil
        end
  ...
  def get_rivalries_with_notifications(user_id) do
    query =
      from(ur in UserRivalry,
        where: ur.user_id == ^user_id and ur.status == "active" and ur.notifications_enabled == true,
        preload: [:rival]
      )
  ...
  Returns a map of round => list of predictions.
  """
  ...
  @doc """
  Get a specific bracket prediction.
  """
  def get_bracket_prediction(user_id, round, position) do
    Repo.get_by(BracketPrediction, user_id: user_id, round: round, position: position)
    |> Repo.preload(:team)
  end

  @doc """
  Set or update a bracket prediction.
  """
  def set_bracket_prediction(%{user_id: user_id, round: round, position: position, team_id: team_id}) do
  ...
  @doc """
  Clear a bracket prediction (set team to nil).
  """
  ...
  @doc """
  Clear all predictions in later rounds when a team is removed from an earlier round.
  ...
  defp get_rounds_after(round) do
    all_rounds = ["round_of_32", "round_of_16", "quarter_final", "semi_final", "final", "winner"]
  ...
  def set_bracket_result(%{round: round, position: position, team_id: team_id}) do
    ...
    case Repo.get_by(BracketResult, round: round, position: position, competition_id: comp_id) do
      nil ->
        %BracketResult{}
        |> BracketResult.changeset(%{round: round, position: position, team_id: team_id, competition_id: comp_id})
        |> Repo.insert()
  ...
  def calculate_bracket_accuracy(user_id) do
    ...
    %{
      ...
      overall_accuracy: if(total_possible > 0, do: total_correct / total_possible * 100, else: 0.0)
    }
  end
